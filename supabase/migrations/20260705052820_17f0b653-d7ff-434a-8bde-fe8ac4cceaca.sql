-- Batch 2.1 hardening:
--   (a) enter_contest_pool_atomic: strengthen defense-in-depth guard so ANY
--       JWT-authenticated caller is rejected. Legitimate calls come from the
--       service-role edge function where auth.uid() IS NULL.
--   (b) admin_override_responsible_gaming: explicitly restate REVOKE/GRANT so
--       ACL no longer relies on CREATE-OR-REPLACE inheritance.
...
-- (b) admin_override_responsible_gaming: explicit ACL restatement.
REVOKE EXECUTE ON FUNCTION public.admin_override_responsible_gaming(uuid, uuid, text, bigint, text)
  FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.admin_override_responsible_gaming(uuid, uuid, text, bigint, text)
  TO service_role;
