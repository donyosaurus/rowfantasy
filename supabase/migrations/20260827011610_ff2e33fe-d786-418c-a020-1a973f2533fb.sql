-- Phase 3a fix batch: adversarial-review findings on the survivor migration
-- 20260827005416. Four fixes, each a CREATE OR REPLACE of the exact current
-- body with only the described change:
--   1. TOCTOU: lock the contest_rounds row in both survivor RPCs.
--   2. Integer overflow in survivor round_no / advance_count validation.
--   3. Survivor fixed-pick-count check must precede the generic min/max checks.
--   4. Survivor "tiers unsupported" must precede generic tier JSON processing.

-- ---------- FIXES 2, 3, 4: admin_create_contest_v2 (23 params, signature unchanged) ----------
...
BEGIN
  ...
  -- FIX 3: survivor fixed-pick-count check precedes the generic min/max checks
  -- so p_min_picks NULL/1 or p_max_picks < p_min_picks reports the survivor
  -- message. Non-survivor calls fall through to the generic checks unchanged.
  ...
  -- FIX 4: survivor tier rejection precedes any generic tier JSON processing
  -- so a scalar/object p_entry_tiers reports the survivor message instead of a
  -- jsonb type error. Non-survivor ordering is unchanged.
  ...
  IF COALESCE(p_primitive, 'placement') = 'survivor' THEN
    ...
    -- FIX 2: bound the magnitude while the values are still known to be
    -- digit-only, so every later ::int cast is safe (numeric casts of
    ...
    -- FIX 2 (races): bound the magnitude before the first ::int cast below.
    IF EXISTS (
      SELECT 1 FROM jsonb_array_elements(p_races) r
...
-- ---------- FIX 1(a): submit_survivor_round_picks ----------
...
BEGIN
  ...
  -- entry is already locked above) and evaluate status/lock_at on the
...
-- ---------- FIX 1(b): score_survivor_round_atomic ----------
...
BEGIN
  ...
  -- FIX 1(b): re-take the target round under a row lock (lock order:
