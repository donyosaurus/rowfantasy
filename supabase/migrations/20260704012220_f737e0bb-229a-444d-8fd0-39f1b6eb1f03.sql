...
BEGIN
  ...
  -- Clear any stale winner_ids left over from scoring so voided pools don't
  -- surface phantom winners in admin views. Voided pools have no winners.
...
-- Retroactively clean up already-voided pools that still carry stale winner_ids.
UPDATE public.contest_pools
SET winner_ids = ARRAY[]::uuid[]
WHERE status = 'voided'
  AND winner_ids IS NOT NULL
  AND array_length(winner_ids, 1) > 0;
