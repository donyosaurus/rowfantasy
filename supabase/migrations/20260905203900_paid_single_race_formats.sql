...
-- Every other paid shape keeps the >=2-race floor. Both bodies are the latest
-- bases restated verbatim with only the described changes.
...
BEGIN
  ...
  -- Phase 4i-1: single-race confidence template. jsonb ->> on a missing key is
  -- NULL, so COALESCE keeps this a non-null boolean.
