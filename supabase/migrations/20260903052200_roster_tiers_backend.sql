-- =====================================================================
-- Phase 4e-1: Tiers backend
--   1. contest_templates.roster_tiers jsonb (nullable, no default)
--   2. admin_create_contest_v2   -> 24th param p_roster_tiers + validation
--   3. enter_contest_pool_atomic -> tier rule for per_competitor entries
-- Both bodies are the current repository bodies (20260903050000_*.sql)
-- restated verbatim with only the described blocks added. When
-- roster_tiers / p_roster_tiers is NULL there is no behavior change.
-- =====================================================================

ALTER TABLE public.contest_templates ADD COLUMN IF NOT EXISTS roster_tiers jsonb;

DROP FUNCTION IF EXISTS public.admin_create_contest_v2(text, text, text, timestamptz, jsonb, jsonb, jsonb, bigint, integer, jsonb, jsonb, boolean, boolean, text, text, uuid, text, text, jsonb, integer, integer, uuid, jsonb);

CREATE FUNCTION public.admin_create_contest_v2(p_name text, p_sport text, p_gender_category text, p_lock_time timestamp with time zone, p_races jsonb, p_competitors jsonb, p_race_entries jsonb, p_entry_fee_cents bigint, p_max_entries integer, p_payout_structure jsonb DEFAULT NULL::jsonb, p_entry_tiers jsonb DEFAULT NULL::jsonb, p_allow_overflow boolean DEFAULT false, p_void_unfilled_on_settle boolean DEFAULT false, p_card_banner_url text DEFAULT NULL::text, p_draft_banner_url text DEFAULT NULL::text, p_contest_group_id uuid DEFAULT NULL::uuid, p_primitive text DEFAULT 'placement'::text, p_roster_mode text DEFAULT 'per_race'::text, p_scoring_config jsonb DEFAULT NULL::jsonb, p_min_picks integer DEFAULT 2, p_max_picks integer DEFAULT 4, _admin_user_id uuid DEFAULT NULL::uuid, p_rounds jsonb DEFAULT NULL::jsonb, p_roster_tiers jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE n.nspname = 'public' AND p.proname = 'admin_create_contest_v2';
