-- CHANGE 1: Drop SECURITY DEFINER from the trigger function (use SECURITY INVOKER default).
-- Also CHANGE 2: insert combined-decrease+stage block at end of Section B.
...
-- CHANGE 3+4: Pending-pair CHECK constraint (drop first if exists for idempotency).
ALTER TABLE public.responsible_gaming
  DROP CONSTRAINT IF EXISTS responsible_gaming_pending_pair_check;

ALTER TABLE public.responsible_gaming
  ADD CONSTRAINT responsible_gaming_pending_pair_check
  CHECK (
    (pending_deposit_limit_monthly_cents IS NULL AND pending_limit_effective_at IS NULL)
    OR
    (pending_deposit_limit_monthly_cents IS NOT NULL AND pending_limit_effective_at IS NOT NULL)
  );

-- CHANGE 5: apply_pending with FOR UPDATE row lock.
