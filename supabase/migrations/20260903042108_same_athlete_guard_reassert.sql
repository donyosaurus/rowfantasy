-- =====================================================================
-- Phase 4b: same-athlete guard for v2 per-race entries AND survivor rounds
--   1. enter_contest_pool_atomic     -> duplicate_competitor on v2 per_race
--   2. submit_survivor_round_picks   -> duplicate_competitor for rounds 2+
--   3. admin_create_contest_v2       -> create-side distinct-competitor floors
-- Each body below is the latest repository body restated verbatim with only
-- the described insertion; legacy (scoring_config IS NULL) validation order and
-- messages are byte-identical and the per_competitor block is untouched.
-- =====================================================================
...
GRANT  EXECUTE ON FUNCTION public.enter_contest_pool_atomic(uuid, uuid, uuid, text, jsonb, text)
  TO service_role;
-- =====================================================================
-- Phase 4b (part 2): submit_survivor_round_picks + admin_create_contest_v2
-- Bodies restated verbatim from the latest repository migrations with only
-- the described insertions.
-- =====================================================================
...
BEGIN
  ...
  IF p_scoring_config IS NOT NULL
     ...
     AND p_scoring_config->>'primitive' IS DISTINCT FROM 'survivor' THEN
    IF (
