-- Phase 4i-1b: single-race exemptions in enter_contest_pool_atomic must be
-- derived from the complete authorized contest shape, not flag presence alone.
-- Restates enter_contest_pool_atomic from 20260905203900_paid_single_race_formats.sql
-- verbatim with exactly two expression changes.
...
BEGIN
  ...
  -- Phase 4i-1b: single-race exemption only applies to the complete authorized
  -- shape: confidence=true AND primitive=placement AND roster_mode=per_race AND exactly 1 race.
  _conf1 := (COALESCE(_scoring_config->>'confidence', '') = 'true')
            AND (COALESCE(_scoring_config->>'primitive', '') = 'placement')
            AND (COALESCE(_roster_mode, 'per_race') = 'per_race')
  ...
  IF _scoring_config IS NOT NULL AND _roster_mode = 'per_competitor' THEN
    -- Phase 4i-1b: the paid >=2-race floor is waived only for placement
    -- per_competitor tier contests; every other per_competitor shape keeps it.
    IF (_roster_tiers IS NULL OR COALESCE(_scoring_config->>'primitive', '') <> 'placement')
       AND (SELECT count(*) FROM contest_races WHERE template_id = _contest_template_id) < 2 THEN
      RETURN QUERY SELECT false, 'insufficient_events'::text, NULL::uuid, NULL::uuid, NULL::integer, NULL::integer, NULL::bigint;
      RETURN;
    END IF;
