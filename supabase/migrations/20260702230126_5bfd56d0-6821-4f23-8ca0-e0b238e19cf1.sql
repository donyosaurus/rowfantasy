...
BEGIN
  ...
  FOR _key, _val IN SELECT * FROM jsonb_each(_old_payout) LOOP
    ...
    _cents_int := (_val)::bigint;
    IF _cents_int < 0 THEN
      RETURN QUERY SELECT false, 'invalid_payout_value'::text, _old_max, _new_max_entries,
                          _old_payout, _old_prize_pool;
      RETURN;
    END IF;
    ...
    _scaled_val := (_cents_int * _new_max_entries) / _old_max;
  ...
  _rounding_dust := ((_old_scaled_total * _new_max_entries) / _old_max) - _sum_cents;
