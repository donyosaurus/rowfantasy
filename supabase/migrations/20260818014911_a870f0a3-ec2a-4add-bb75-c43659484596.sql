...
BEGIN
  ...
  SELECT id, min_picks, max_picks INTO _template_check, _min_picks, _max_picks
  ...
  -- Every pick must exist in the target pool's crew list. Runs BEFORE any
  -- wallet debit / transaction / ledger insert so a rejected entry moves no money.
  SELECT count(*) INTO _invalid_pick_count
  FROM jsonb_array_elements(_picks) AS elem
  WHERE NOT EXISTS (
    SELECT 1 FROM contest_pool_crews cpc
